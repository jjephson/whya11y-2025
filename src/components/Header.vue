<template>
	<header class="site-header">
		<div class="top-container">
			<a href="/" class="logo-link" aria-label="WhyA11y - Go to home page">
				<span class="logo-bg">
					<img src="/images/design/logo.png" alt="" class="logo-img" width="70" height="35" />
				</span>
			...
			>
				...
				<div class="toggle-icon">
					<svg v-if="isDark" class="sun-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" role="img" aria-hidden="true">
					...
					<svg v-else class="moon-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" role="img" aria-hidden="true">
		...
		<nav id="navigation" aria-label="Main navigation">
			<ul class="menu" id="main-menu" role="menubar">
				<li v-for="link in menuLinks" :key="link.href" role="none">
					<a
						:href="link.href"
					...
					>
						{{ link.label }}
						<span v-if="currentPath === (link.href === '/' ? '/' : link.href.replace(/\/$/, ''))" class="sr-only">(current page)</span>
					</a>
				</li>
			</ul>
		</nav>
...
<script setup>
	...
	function setTheme(dark) {
		...
		if (announcements) {
			announcements.textContent = `Switched to ${dark ? 'dark' : 'light'} mode`;
		}
	}
